...
\subsection*{Inception Layers}
Inception Layers wurden 2014 das erste mal in GoogLeNet verwendet \cite{Inception_paper}. In einem normalen Convolutional Layer muss man sich festlegen welche Größe der Filterkernel haben soll. Verschieden Größen bringen dabei immer Vor- und Nachteile mit sich. Deshalb ist die Idee von Inception Layers diese Wahl flexibler zu machen und dem Netz zu überlassen. Um das zu ermöglichen enthält ein Inception Layer Convolution Module mit verschiedenen Kernelgrößen und auch pooling Module, die Konkateniert werden.
In \Cref{fig:inception_naive_graph} sieht man den Naiven Aufbau eines Inception Layers, wobei alle Module direkt auf den Input angewendet und dann Konkateniert werden. Das Problem bei diesem Ansatz ist allerdings, das dieses Layer einen extrem hohen Rechenaufwand hätte. Um das ein wenig zu verringern führt man üblicherweise vor den großen Convolutions jeweils auch eine 1x1 Convolution durch. Durch die 1x1 Convolution reduziert man die Dimension des Inputs für die großen Module, was zu einer deutlich besseren Laufzeit führt. Dadurch erhält man den Aufbau, den man in \Cref{fig:inception_pro_graph} sieht.
\begin{figure}
	\caption{Naives Inception Layer}
	\includegraphics[width=\textwidth]{images/inception_naive_graph.png}
	\label{fig:inception_naive_graph}
\end{figure}
\begin{figure}
	\caption{Verbessertes Inception Layer}
	\includegraphics[width=\textwidth]{images/inception_pro_graph.png}
	\label{fig:inception_pro_graph}
\end{figure}
\subsubsection*{Aufbau Klassisches CNN}
\begin{figure}
...
\subsubsection*{Aufbau CNN mit Inception Layer}
\begin{figure}
...
\end{figure} \Cref{fig:main_graph_inception} zeigt nun den Aufbau eines Netzes in dem das convolutional Layer und pooling Layer durch ein inception Layer ersetzt wurde. Dieses bekommt den gleichen Input wie das Convolution Layer im vorher beschriebenen Netz. Der Output des Inception Layers sind 4*32 features. Jedes der 4 Module des Inception Layers erzeugt 32 features der Größe 28x28, welche dann konkateniert werden. Diese Features werden dann wie vorher durch zwei fully-connected Layers weiter verarbeitet.
